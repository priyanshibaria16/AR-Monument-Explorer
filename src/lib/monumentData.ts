...
export const getUserProgress = (): UserProgress => {
  const progress = localStorage.getItem('monumentProgress');
...
export const updateUserProgress = (progress: UserProgress) => {
  localStorage.setItem('monumentProgress', JSON.stringify(progress));
};

export const checkAndUnlockAchievements = (progress: UserProgress): string[] => {
  ...
  if (progress.visitedMonuments.length >= 5 && !progress.achievements.includes('monument-collector')) {
...
export const toggleFavorite = (monumentId: string) => {
  const progress = getUserProgress();
  const index = progress.favorites.indexOf(monumentId);
...
export const shareMonument = (monument: Monument, platform: string) => {
  const progress = getUserProgress();
  
  if (!progress.achievements.includes('social-sharer')) {
    progress.achievements.push('social-sharer');
    updateUserProgress(progress);
  }
